...
export const PanelBig = ({ classNameTitle, title, children, filtersChildren, className, contentClassName }: PanelBigProps) => {
    return (
        <Panel className={cn("py-0 w-full h-full flex flex-col max-w-[680px] xl:max-w-[780px] 2xl:max-w-[852px]", className)}>
            ...
            <CardContent className={cn("flex flex-col px-4 lg:px-6 flex-1 w-full h-full", contentClassName)}>
                {children}
            </CardContent>
        </Panel>
    )
