...
const MapSegments = () => {
    ...
    const { labels, values } = segments?.reduce(
        (acc: { labels: string[]; values: number[] }, segment: { nome: string; valor: number }) => {
    ...
    return (
        ...
        >
            ...
            <Button className="sidebar-link-active rounded-2xl">{t("DashboardPage.analyzeSegments")}</Button>
        </PanelBig>
    );
};
